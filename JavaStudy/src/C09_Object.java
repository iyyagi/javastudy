...
public class C09_Object {
	/*
	  ...
	  - Object 클래스의 메서드는 주로 해당 클래스에 맞는 형태로 오버라이드 하여 사용한다.
	  
	  # Object.toString()
	  - '해당 인스턴스가 문자열로 표현된다면 어떤 모양이여야 하는가'를 정의한 메서드
	  - 주로 print() 메서드들이 해당 인스턴스의 toString()을 호출하도록 설계되어 있다.
	  - toString()의 기본 동작은 해당 인스턴스의 클래스 이름과 주소값을 출력하는 것이다.
	  
	  # Object.equals(obj) 
	  - '두 인스턴스가 같다고 할 수 있는 기준'을 정의해놓은 메서드 
	*/
	...
	public static void main(String[] args) {
		...
		System.out.println("==비교의 결과  : " + (g1 == g2)); // 주소를 비교
		System.out.println("equals 결과 : " + g1.equals(g2)); // 오버라이드 하여 지정한 값으로 비교
